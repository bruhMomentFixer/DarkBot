...
import com.LouDBasS_101.KillListener;
...
public class SmartKillAndCollect implements Behavior, Listener {
    ...
    public void listenKill(LogMessageEvent event) {
        ...
        if(event.getMessage().contains("no recibes ninguna") || 
        (event.getMessage().contains("Has recibido") && collectorModule.currentBox != null)) { 
